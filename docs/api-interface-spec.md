...
Frontend (`http://localhost:5173`) ↔ Backend（動的ポート）間のインターフェース仕様。
...
| 項目 | 値 |
|------|-----|
| Allow Origins | `http://localhost:5173`, `http://127.0.0.1:5173`（`KANATA_ALLOWED_ORIGINS` 環境変数で追加可） |
| Allow Origin Regex | `file://.*`（Electron プロダクション用） |
...
| HTTP | detail | 原因 |
|------|--------|------|
| 404 | `"No fundamentals for {symbol}"` | データが存在しない |
...
| Fundamentals ルート | `backend/src/routes/fundamentals.py` |
...
| Watchlist API クライアント | `apps/renderer/src/lib/watchlistApi.ts` |
| Quotes / Fundamentals API クライアント | `apps/renderer/src/lib/api.ts` |
| Search API クライアント | `apps/renderer/src/lib/searchApi.ts` |
| TypeScript 型定義 | `apps/renderer/src/types.ts` |
| Watchlist フック | `apps/renderer/src/hooks/useWatchlists.ts` |
| Chart データフック | `apps/renderer/src/hooks/useChartData.ts` |
| Search フック | `apps/renderer/src/hooks/useDebouncedSearch.ts` |
