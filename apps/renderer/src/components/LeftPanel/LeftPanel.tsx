...
function ToolBtn({ id, label, icon, activeTool, setTool }: {
  id: string; label: string; icon: string; activeTool: string; setTool: (id: string) => void;
}) {
  const active = activeTool === id;
  return (
    <button className={`tool-btn${active ? ' active' : ''}`} onClick={() => setTool(id)} title={label}>
...
function Toggle({ label, value, onChange, color }: {
  ...
  return (
    <label className={`toggle-row${value ? ' on' : ''}`}>
      <span className="toggle-dot" style={{ background: value ? (color || 'var(--accent)') : 'transparent', borderColor: color || 'var(--accent)' }} />
...
export function LeftPanel({ state, setState }: LeftPanelProps) {
  const setTool = (t: string) => setState(s => ({ ...s, activeTool: s.activeTool === t ? 'pan' : t }));
  const setInd = (k: keyof AppState['indicators'], v: boolean) => setState(s => ({ ...s, indicators: { ...s.indicators, [k]: v } }));
  const setFin = (k: keyof AppState['financial'], v: boolean) => setState(s => ({ ...s, financial: { ...s.financial, [k]: v } }));
    ...
    <aside className="panel panel-left">
      <Section title="TIMEFRAME">
        <div className="tf-grid">
          {timeframes.map(tf => (
            <button key={tf} className={`tf-btn${state.timeframe === tf ? ' active' : ''}`}
      ...
      <Section
        title="DRAWING TOOLS"
        right={<button className="link-btn" onClick={clearDrawings} title="Clear all drawings">CLEAR</button>}
      >
        <div className="tool-grid">
          {tools.map(t => <ToolBtn key={t.id} {...t} activeTool={state.activeTool} setTool={setTool} />)}
        </div>
      ...
      <Section title="TECHNICAL ANALYSIS">
        ...
        <Toggle label="SMA 5" value={state.indicators.sma5} onChange={v => setInd('sma5', v)} color="var(--amber)" />
        <Toggle label="SMA 25" value={state.indicators.sma25} onChange={v => setInd('sma25', v)} color="var(--accent)" />
        <Toggle label="SMA 75" value={state.indicators.sma75} onChange={v => setInd('sma75', v)} color="var(--magenta)" />
        <Toggle label="EMA 20" value={state.indicators.ema20} onChange={v => setInd('ema20', v)} color="var(--lime)" />
        <div className="subheader">Overlays</div>
        <Toggle label="Bollinger Bands 20,2" value={state.indicators.boll} onChange={v => setInd('boll', v)} color="oklch(0.75 0.07 220)" />
        <Toggle label="Parabolic SAR" value={state.indicators.psar} onChange={v => setInd('psar', v)} color="oklch(0.78 0.20 350)" />
      ...
      <Section title="FUNDAMENTALS">
        <Toggle label="Show fundamentals pane" value={state.showFinancial} onChange={v => setState(s => ({ ...s, showFinancial: v }))} color="var(--lime)" />
        {state.showFinancial && (
          <div style={{ marginTop: 6 }}>
            <Toggle label="ROE (%)" value={state.financial.roe} onChange={v => setFin('roe', v)} color="var(--lime)" />
            <Toggle label="ROIC (%)" value={state.financial.roic} onChange={v => setFin('roic', v)} color="var(--teal)" />
            <Toggle label="PER (×)" value={state.financial.per} onChange={v => setFin('per', v)} color="var(--amber)" />
          </div>
        )}
      </Section>

      <Section title="VOLUME">
        <Toggle label="Show volume" value={state.showVolume} onChange={v => setState(s => ({ ...s, showVolume: v }))} color="var(--muted)" />
      </Section>
    </aside>
  );
}
