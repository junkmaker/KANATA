...
- **スコア帯は固定値ではなく分位で切る**（`score_bands`）。満点は加点要素の構成に依存する — `TREND_BONUS` を 0 にした時点で満点が 100 → 75 になり、固定帯の 85-100 が n=0 の空行になった。ただし**件数の分位を素朴に取ると境界が最頻値に重複して帯が 1 本に潰れる**（スコアは離散値で最頻値に集中する）ので、出現値が少なければ 1 値 1 帯、それでも足りなければ値の分位へ落とす 3 段構え。要求本数より減ったら §2 に警告を出す（黙って出すと「単調に増えている」と読める 1 行になる）。UI 側の `MIN_SCORE_OPTIONS`（`ScreeningView.tsx`）も満点に合わせる — 満点超えの選択肢は選んだ瞬間に必ず空テーブルになる
